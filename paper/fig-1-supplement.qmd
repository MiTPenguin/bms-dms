...
```{r Missing-variants, fig.width=8.5, fig.height=5.5}
full_grid <- aalet %>% 
  bind_rows(tibble(thr = "Stop", aa = "*")) %>% 
  filter(!aa %in% c("Z", "B")) %>% 
  expand_grid(pos = 1:1187,
              facet_label = levels(merge_sumstats$facet_label)) 

merge_sumstats %>%
  full_join(full_grid) %>% 
  group_by(facet_label) %>%
  # filter(facet_label == "Abundance") %>%
  summarise(missing_variants = sum(is.na(z_statistic)),
            total_variants = n(),
            missing_pct = 100 * missing_variants / total_variants) 

merge_sumstats %>%
  full_join(full_grid) %>% 
  group_by(aa,pos) %>% 
  # filter(facet_label == "Abundance") %>%
  summarise(missing_vars = sum(!is.na(z_statistic)) == 0,
            .groups = "drop") %>%
  summarise(fraction = sum(missing_vars)/ length(missing_vars))
            # total_variants = n(),
            # missing_pct = 100 * missing_variants / total_variants) 

missing_variants <- merge_sumstats %>%
  full_join(full_grid) %>% 
  # filter(is.na(z_statistic)) %>%
  mutate(missing_variant = if_else(is.na(z_statistic), "salmon","white")) %>% 
  ...
  geom_tile(aes(x = pos, y = aa, fill = missing_variant)) +
  # scale_fill_scico_mid("Z statistic", limits = c(-10, 2.5), palette = "vik", mid = 0,
  #                      na.value = "purple",
  #                      oob = scales::squish) +
  ggtitle("Missing variants") +
  scale_fill_identity() +
  scale_x_continuous(
...
```{r fig-S1x2-merged, fig.width=8.5, fig.height=8}
layout <- c(
  area(1, 1, 1, 1),
  area(1, 2, 1, 2),
  area(1, 3, 1, 3),
