...
assay4 <- read_tsv("sumstats/TYK2-run4-combined-cleaned.sumstats.tsv", show_col_types = FALSE) %>%
  mutate(assay = "assay4")

assay7 <- read_tsv("sumstats/TYK2-run7-combined-cleaned.sumstats.tsv", show_col_types = FALSE) %>%
  mutate(assay = "assay7")

assay_data <- bind_rows(assay3, assay4, assay7)  %>%
    group_by(assay) %>%
...
treated <- assay_data %>%
  ...
  mutate(condition = if_else(condition == "IFNalphaWithDrug_100",
                             "IFNalpha+BMS-986202_1e-05",
                             condition))
...
untreated <- assay_data %>%
  ...
  select(-condition, -statistic, -p.value, -fdr, -version)
...
write_tsv(contrast_sumstats, "sumstats/TYK2-combined-cleaned.contrast.tsv")
```
