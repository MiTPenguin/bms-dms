...
ifna_signaling <- contrast_sumstats %>%
    filter(condition == "IFNalpha100_0", assay == "assay3") %>%
    rename("p.adj_dms" = "fdr",
      "DMS Statistic" = "statistic")
...
ggplot(signal_stab) +
  ...
  scale_color_manual(values = c("signaling-only LOF" = "blue", "other" = "gray")) +
...
ggplot(pos_counts) +
  ...
  coord_flip() +
  scale_x_reverse(breaks = seq(0, 1100, by = 100)) +
  xlab("TYK2 Position") +
  ylab("Number of\nSignaling-Only\nLOF Variants")
```
