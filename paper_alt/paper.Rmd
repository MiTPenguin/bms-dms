...
```{r loading}
assay1 <- read_tsv("../sumstats_alt/TYK2-run1.sumstats.tsv", show_col_types = FALSE) %>%
  mutate(assay = "assay1") %>%
  mutate(aa = if_else(aa %in% c("X", "Stop", "*"), "Stop", aa))

assay2 <- read_tsv("../sumstats_alt/TYK2-run2.sumstats.tsv", show_col_types = FALSE) %>%
  mutate(assay = "assay2") %>%
  mutate(aa = if_else(aa %in% c("X", "Stop", "*"), "Stop", aa))

assay3 <- read_tsv("../sumstats_alt/TYK2-run3.sumstats.tsv", show_col_types = FALSE) %>%
  mutate(assay = "assay3") %>%
  mutate(aa = if_else(aa %in% c("X", "Stop", "*"), "Stop", aa))

flow_data <- read_tsv("../sumstats_alt/TYK2-vamp2.midpoints.tsv", show_col_types = FALSE) %>%
  mutate(aa = if_else(aa %in% c("X", "Stop", "*"), "Stop", aa))
...
write_tsv(contrast_sumstats, "../sumstats_alt/TYK2.contrast.tsv")
```
