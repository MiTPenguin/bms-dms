...
```{r loading}
assay1 <- read_tsv("../sumstats/archive/TYK2-run3-combined-cleaned.sumstats.tsv", show_col_types = FALSE) %>%
  mutate(assay = "assay1") %>% select(-version) %>%
  mutate(aa = if_else(aa %in% c("X", "Stop", "*"), "Stop", aa))

assay2 <- read_tsv("../sumstats/archive/TYK2-run4-combined-cleaned.sumstats.tsv", show_col_types = FALSE) %>%
  mutate(assay = "assay2") %>% select(-version) %>%
  mutate(aa = if_else(aa %in% c("X", "Stop", "*"), "Stop", aa))

assay3 <- read_tsv("../sumstats/archive/TYK2-run7-combined-cleaned.sumstats.tsv", show_col_types = FALSE) %>%
  mutate(assay = "assay3") %>% select(-version) %>%
  mutate(aa = if_else(aa %in% c("X", "Stop", "*"), "Stop", aa))

flow_data <- read_tsv("../sumstats/archive/TYK2-FLOW-flow-cleaned.midpoints.tsv", show_col_types = FALSE) %>%
  mutate(aa = if_else(aa %in% c("X", "Stop", "*"), "Stop", aa))
...
write_tsv(contrast_sumstats, "../sumstats/archive/TYK2.contrast.tsv")
```
